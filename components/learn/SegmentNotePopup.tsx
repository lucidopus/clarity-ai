...
import { Play, Trash2, Bold, Italic, List, Code } from 'lucide-react';
          ...
          >
            ...
            <div className="flex items-center justify-between gap-3 px-5 pt-4 pb-3">
              ...
              <button
                type="button"
                onClick={onClose}
                className="font-mono text-[10px] px-1.5 py-1 rounded transition-colors hover:opacity-80"
                style={{
              ...
              <div className="flex items-center gap-1.5">
                {isExisting && onDelete && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-[12px] rounded-md transition-colors hover:bg-background"
                    style={{ color: '#ef4444' }}
                    title="Delete note"
                  >
                    <Trash2 size={12} />
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="px-2.5 py-1.5 text-[12px] rounded-md transition-colors hover:bg-background"
                ...
                <button
                  ...
                  onClick={handleSave}
                  disabled={!canSave}
                  className="inline-flex items-center gap-2 pl-3 pr-2 py-1.5 text-[12px] rounded-md font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
                  style={{ background: 'var(--accent)', color: '#06080d' }}
                >
                  Save note
                  <span
                    className="font-mono px-1.5 py-0.5 rounded text-[10px]"
                    style={{ background: 'rgba(6,8,13,0.18)', color: 'rgba(6,8,13,0.85)' }}
                  >
                    ⌘↵
                  </span>
                </button>
...
function FmtButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      title={title}
      className="w-7 h-7 grid place-items-center rounded-md transition-colors hover:bg-background"
      style={{
